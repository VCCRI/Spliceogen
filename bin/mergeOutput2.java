...
public class mergeOutput2 {
        ...
        try {
            ...
            while ((s = in.readLine()) != null && s.length() != 0) {
                ...
                if(nextVariant && !prevID[4].equals("GENE")) {
                    ...
                    String[] lrScores = calculateLogRegScores(scores, out[6]).split("\\s+");
                    out[19]= lrScores[0];                
                    out[20]= lrScores[1];
                    out[21]= lrScores[2];                
                    out[22]= lrScores[3];
    ...
    public static void writeHeaders (String fileName) {
        ...
        try {
            ...
            BufferedWriter avWriter = new BufferedWriter(fwAV);
            avWriter.write("#CHR\tSTART\tREF\tALT\tGENE\twithinSite\tESEmaxRef\tESEmaxAlt\tESSminRef\tESSminAlt\tdonGain\taccGain\tdonLoss\taccLoss"+"\n");
            avWriter.close();
        } catch (IOException e) {
            System.out.println(e.getMessage());
        }
    ...
    public static void outputVariantToBuffers(String[] out) {	
        ...
        String avLine = out[0]+"\t"+out[1]+"\t"+out[3]+"\t"+out[4]+"\t"+out[5]+"\t"+out[6]+"\t"+out[15]+"\t"+out[16]+"\t"+out[17]+"\t"+out[18]+"\t"+out[19]+"\t"+out[20]+"\t"+out[21]+"\t"+out[22];
